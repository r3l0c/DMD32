...
void DMD::writePixel(unsigned int bX, unsigned int bY, uint8_t bGraphicsMode, uint8_t bPixel)
{
    ...
    case GRAPHICS_NOR:
        ...
        if ((bPixel == true) && ((bDMDScreenRAM[uiDMDRAMPointer] & lookup) == 0))
            bDMDScreenRAM[uiDMDRAMPointer] |= lookup; // one bit is pixel off
        break;
    }
}
    ...
    for (ui = 0; ui < numPixels; ui++)
    {
        ...
        case PATTERN_ALT_0: // every alternate pixel, first pixel on
            if ((ui & pixelsWide) == 0)
                ...
                // odd row
                writePixel((ui & (pixelsWide - 1)), ((ui & ~(pixelsWide - 1)) / pixelsWide), GRAPHICS_NORMAL,
                           !(ui & 1));
            break;
...
{
    // if PIN_OTHER_SPI_nCS is in use during a DMD scan request then scanDisplayBySPI() will exit without conflict! (and
    // skip that scan)
